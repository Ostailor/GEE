...
#--- Functions ---
# mask data using quality band
def maskQC(img):
    # extract the bits bbb from bitstart to bitstart+bitlen.
    # When bbb is smaller than thres then qualified
...
# mask out values outside the valid range for clear_sky_fraction
# modify the clear-sky-cov values >1 into 1
...
# mask A3 band using quality band
def maskA3(img, cur_qc):
    return img.updateMask(img.select(cur_qc).eq(0))

# Match MCD43A3 and MCD43A2 product together
def Match_mcd(img):
...
# QA and snow mask for MCD
def maskSnowfree(image):
    mask2 = image.select('BRDF_Albedo_LocalSolarNoon').lt(85) #small sza
    mask3 = image.select('Snow_BRDF_Albedo').eq(0) # snow free
...
vvars = ['ALBEDO']
#vvars = ['ALBEDO', 'ET', 'LAI', 'LST_day', 'LST_night', 'NDVI']
#vvars = ['AIRT']
#vvars = ['PRCP_GPM']
#datebeg = datetime.date(2003, 1, 1)
#datebeg = datetime.date(2024, 12, 1)
datebeg = datetime.date(2024, 12, 1)
#dateend = datetime.date(2023, 2, 1)
dateend = (datetime.date.today().replace(day=1) - datetime.timedelta(1)).replace(day=1)
ndays = (dateend - datebeg).days + 1
dir_root0 = '/Users/omtailor/Google Drive/My Drive/Team/Data'
...
for vvar in vvars:
    ...
    for dd in range(ndays):
        ...
        # get start/end date for the cur_month
        cur_dateStart = date.strftime('%Y-%m-%d')
        #print(f"{cur_dateStart=}")
        ...
        if not determine_complete(date_last, date, FREQUENCY): continue
        # Process each band
        for cur_band_idx in range(len(LS_BAND)):
            ...
            if vvar == 'ALBEDO':
                ...
                #cur_mod = mod.map(lambda img: maskA3(img, 'Snow_BRDF_Albedo'))
                # mod_si must be processed before mod_sf. The formmer is masked with solar angle only while the latter is masked by two.
                #mod_si = cur_mod.map(maskSnowincluded).select(cur_band)
                #mod_sf = cur_mod.map(maskSnowfree).select(cur_band)
                ...
                # sum hourly to daily for PRCP_GPM only
                if vvar == 'PRCP_GPM':
                    ee_startDate = ee.Date(cur_dateStart)
                    ee_endDate = ee.Date(cur_dateEnd)
                    numberOfDays = ee_endDate.difference(ee_startDate, 'days')
                    cur_mod = ee.ImageCollection(ee.List.sequence(0, numberOfDays.subtract(1)).map(lambda dayOffset: sumHourly(dayOffset, cur_mod, ee_startDate, ee_endDate)))
                mods = {vvar:cur_mod}
            for vvar2 in mods.keys():
                ...
                if len(LS_BAND_QC) > 0:
                    ...
                    cur_mod = mods[vvar2]
                # select data in valid range
                if vvar == 'CLEAR_SKY':
                    cur_mod = cur_mod.map(maskValidRange_Uplimit)
                else:
                    cur_mod = cur_mod.map(maskValidRange)
                ## change ERA Precipitation unit from m to mm
                #if vvar2 == 'PRCP_ERA':
                #    cur_mod = cur_mod.map(applyScaler)
                # resampling
                #print(f"{RESAMPLING=}")
                if RESAMPLING:
                ...
                for ccase in cc:
                    ...
                    task.start()
                time.sleep(10) # to make sure the task is submitted


#vvars = ['AIRT', 'ALBEDO', 'ALBEDO_sf', 'ET', 'LAI', 'LST_day', 'LST_night', 'NDVI', 'PRCP_GPM']
vvars = ['ALBEDO']
fillvalue = -32768
datebeg = datetime.date(2024, 12, 1)
dateend = (datetime.date.today().replace(day=1) - datetime.timedelta(1)).replace(day=1)
...
for vvar in vvars:
    conf = conf0[vvar]
    for dummy in conf: exec('{0} = conf["{0}"]'.format(dummy))
    cres = '{}km'.format(str(round(OUTPUT_SCALE/1000)))
    dir_root = os.path.join(dir_root0, 'monthly_{}'.format(cres))
    dir_data = os.path.join(dir_root, vvar, 'Data')
    dummy = FREQUENCY.split('-')
    ...
    for dd in range(ndays):
        ...
        if os.path.exists(fname): continue
        fname_mean = os.path.join(dir_in, '{0}/{1}_{2}_mean_{3}.tif'.format(date.strftime('%Y'), vvar, date.strftime('%Y%m'), cres))
        print(f"{fname_mean=}")
        if not os.path.exists(fname_mean): continue
        fname_num = os.path.join(dir_in, '{0}/{1}_{2}_num_{3}.tif'.format(date.strftime('%Y'), vvar, date.strftime('%Y%m'), cres))
        if not is_monthly and not os.path.exists(fname_num): continue
        fname_std = os.path.join(dir_in, '{0}/{1}_{2}_std_{3}.tif'.format(date.strftime('%Y'), vvar, date.strftime('%Y%m'), cres))
        if not is_monthly and not os.path.exists(fname_std): continue
        # read data
        with rasterio.open(fname_mean) as tif_reader1:
            cur_mean = tif_reader1.read(1) * conf['SCALE_FACTOR'] + conf['OFFSET_FACTOR']
        #cur_mean = cur_mean.round()
        #cur_mean[np.isnan(cur_mean)] = fillvalue
        #cur_mean = cur_mean.astype(np.int16)
        if is_monthly:
            ccs = ['mean']
        else:
            ccs = ['mean', 'std', 'num']
            ...
            #cur_std = cur_std.round()
            #cur_std[np.isnan(cur_std)] = fillvalue
            #cur_std = cur_std.astype(np.int16)
            with rasterio.open(fname_num) as tif_reader3:
                cur_num = tif_reader3.read(1)
            #cur_num = cur_num.astype(np.int16)
